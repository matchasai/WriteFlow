...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try{
          ...
          if (data.success) {
            const now = Date.now();
            const tokenExpiry = now + (12 * 60 * 60 * 1000); // 12 hours from now
            
            setToken(data.token);
            localStorage.setItem('adminToken', data.token);
            localStorage.setItem('tokenExpiry', tokenExpiry.toString());
            localStorage.setItem('sessionInitTime', now.toString());
            ...
            if (error.response?.status === 429) {
                ...
                toast.error(`Too many login attempts. Try again in ${retryAfter} seconds.`);
            } else {
